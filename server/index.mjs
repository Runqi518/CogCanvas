...
app.delete('/api/materials/:id', async (req, res) => {
  const material = db.prepare('SELECT source_url FROM materials WHERE id = ?').get(req.params.id);
  db.prepare('DELETE FROM materials WHERE id = ?').run(req.params.id);
  if (material?.source_url?.startsWith('/uploads/')) {
    await unlink(join(uploadDir, basename(material.source_url))).catch((error) => {
